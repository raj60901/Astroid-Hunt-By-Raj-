...
function draw() {
  ...
  //GAMESTATE=end
    ...
    text("PRESS f5 to restart😊",canvas.width/2-400,canvas.height/2+300);
    
  }


  //CREATING THE FIRST GAMEPAGE TO START THE GAME AND THE INSTRUCTIONS
  if(gameState === instruction) {
    stroke("white");
    fill("white");
    textFont("trebuchetMS")
    textSize(50);
    text("------Astorid Hunt(By Raj)------",canvas.width/2-300,canvas.height/2-300);
    ...
    //PROVIDING KEY TO START THE GAME
    if(keyDown("s")) {
      gameState = play;
    } 
    if(keyDown("r")) {
